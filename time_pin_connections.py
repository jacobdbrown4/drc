...
def check_by_pin_connections(original_netlist,modified_netlist,suffix,organ_name):
    ...
    original_non_leafs.update({original_netlist.top_instance.reference.name:list(x for x in original_netlist.top_instance.reference.children if (organ_name not in x.name and 'COMPLEX' not in x.name))})
